...
<div class="container">
<a href="GoToInsUniversitytables.naah" class="btn btn-success radius f-r mt-20">录入信息</a>
<h2>[益起来·希望工程圆梦行动奖学金申请信息管理]</h2>
	<div class="line"></div>
	<form action="SelUniversitytables.naah" method="post" name="sel">
...
<table class="table table-border table-bordered table-hover table-bg mt-10">
		...
		<tr>
			...
			<a href="GoToSelUniversitytables.naah?id=${son.id}" target="_blank" class="btn btn-default radius" >查看</a>&nbsp;&nbsp;&nbsp;
			<input type="button" value="上报" class="btn btn-success radius" onclick="upload('${son.id}',1)" >&nbsp;&nbsp;&nbsp;
			<a href="GoToUpdUniversitytables.naah?id=${son.id}" class="btn btn-warning radius" >修改</a>&nbsp;&nbsp;&nbsp;
			...
			<input type="button" value="上报" class="btn btn-success radius" onclick="upload('${son.id}',1)" >&nbsp;&nbsp;&nbsp;
			<a href="GoToUpdUniversitytables.naah?id=${son.id}" class="btn btn-warning radius" >修改</a>&nbsp;&nbsp;&nbsp;
			<a href="javascript:showreason('${son.id}','Universitytable')" class="btn btn-danger  radius" >驳回原因</a>&nbsp;&nbsp;&nbsp;
			<a href="javascript:deleteunit('${son.id}')" class="btn btn-danger  radius" >删除</a>&nbsp;&nbsp;&nbsp;
			</td>
			</naah:if>
	...
	if(status=='3')
		{
		 ...
		 (
			...
			{
			    layer.close(index);
			    window.location.href="UploadUniversitytables.naah?id="+str+"&&status="+status+"&&reason="+text;
			}
			);
		}
	else
		{
		window.location.href="UploadUniversitytables.naah?id="+str+"&&status="+status;
		}
}
...
function deleteunit(str)
{
	if(confirm("你确定要删除吗？"))
	{
		window.location.href='DelUniversitytables.naah?id='+str;
	}
}

...
<div class="container">
<a href="GoToInsYouthmentalitytables.naah" class="btn btn-success radius f-r mt-20">录入信息</a>
<h2>[青少年心理咨询信息管理]</h2>
	<div class="line"></div>
	<form action="SelYouthmentalitytables.naah" method="post" name="sel">
	...
	<td>
	人员学校：
	<input type="text" class="input-text radius size-M" placeholder="" name="school" id="school" value="${requestScope.school}">
	</td>
...
<table class="table table-border table-bordered table-hover table-bg mt-10">
		...
		<tr>
			...
			<td class="col2 text-c">${son.askDirection}</td>
			...
			<a href="GoToSelYouthmentalitytables.naah?id=${son.id}" target="_blank" class="btn btn-default radius" >查看</a>&nbsp;&nbsp;&nbsp;
			<input type="button" value="上报" class="btn btn-success radius" onclick="upload('${son.id}',1)" >&nbsp;&nbsp;&nbsp;
			<a href="GoToUpdYouthmentalitytables.naah?id=${son.id}" class="btn btn-warning radius" >修改</a>&nbsp;&nbsp;&nbsp;
			...
			<input type="button" value="上报" class="btn btn-success radius" onclick="upload('${son.id}',1)" >&nbsp;&nbsp;&nbsp;
			<a href="GoToUpdYouthmentalitytables.naah?id=${son.id}" class="btn btn-warning radius" >修改</a>&nbsp;&nbsp;&nbsp;
			<a href="javascript:showreason('${son.id}','Youthmentalitytable')" class="btn btn-danger  radius" >驳回原因</a>&nbsp;&nbsp;&nbsp;
			<a href="javascript:deleteunit('${son.id}')" class="btn btn-danger  radius" >删除</a>&nbsp;&nbsp;&nbsp;
			</td>
			</naah:if>
	...
	if(status=='3')
		{
		 ...
		 (
			...
			{
			    layer.close(index);
			    window.location.href="UploadYouthmentalitytables.naah?id="+str+"&&status="+status+"&&reason="+text;
			}
			);
		}
	else
		{
		window.location.href="UploadYouthmentalitytables.naah?id="+str+"&&status="+status;
		}
}
...
function deleteunit(str)
{
	if(confirm("你确定要删除吗？"))
	{
		window.location.href='DelYouthmentalitytables.naah?id='+str;
	}
}
